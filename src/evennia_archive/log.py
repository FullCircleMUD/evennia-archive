...
Every line the library emits goes to its own ``archive.log``, co-located
with Evennia's other logs under ``settings.LOG_DIR``, so debugging an
archive or a restore means reading one file rather than picking its lines
out of the main server log.
...
the server logs use — so an archive line and a `server.log` line can be
...
bootstrapped — ``archive_log`` is a silent no-op. The import is lazy and an
...
_LOG_FILENAME = "archive.log"
_VALID_LEVELS = ("INFO", "WARN", "ERROR")


def archive_log(message: str, level: str = "INFO", trace: bool = False) -> None:
    """Emit one line to ``archive.log``.
